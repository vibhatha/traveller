...
class BaseTrendAnalyzer:
    ...
    def transform_data(self, year):
        ...
        for month in months:
            ...
            self.data_frame[month] = pd.to_numeric(
                self.data_frame[month].str.replace(",", ""), errors="coerce"
            )
        ...
        self.data_frame = self.data_frame[
            self.data_frame["Country"].notna() & self.data_frame["Country"].str.strip().astype(bool)
        ]

        # Add a Year column if not present
        if "Year" not in self.data_frame.columns:
            self.data_frame["Year"] = (
                year  # Adjust this logic to dynamically assign years if needed
            )

        # Convert wide format to long format
        self.data_frame = self.data_frame.melt(
            id_vars=["Country", "Year"], value_vars=months, var_name="Month", value_name="Arrivals"
        )

        # Map month names to numbers for sorting
        month_order = months
        self.data_frame["Month"] = pd.Categorical(
            self.data_frame["Month"], categories=month_order, ordered=True
        )
...
class MonthlyTrendAnalyzer(BaseTrendAnalyzer):
    def analyze_trend(self, year):
        ...
        trend = (
            self.data_frame.groupby(["Year", "Month", "Country"])["Arrivals"].sum().reset_index()
        )
        return trend
